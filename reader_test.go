...
func fixReader(t *testing.T) *depot.Reader {
	t.Helper()
	...
	r, err := depot.NewReader(t.Context(), obj, nil)
	if err != nil {
		t.Fatal("unexpected error", err)
	}
...
func fixMultiReader(t *testing.T) *depot.Reader {
	t.Helper()
	...
	r := depot.MultiReader(t.Context(), []*bfs.Object{obj, obj}, nil)
	t.Cleanup(func() {
		_ = r.Close()
	})
